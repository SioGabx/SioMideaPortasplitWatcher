...
namespace SioMideaPortasplitWatcher
{
    internal class Program
    {
        ...
        private static async Task WatchLoopAsync()
        {
            ...
            var obiCheckerMP = new ObiDeStockChecker("Midea Portasplit","8620890"); //https://www.obi.de/p/8620890/midea-mobile-split-klimaanlage-portasplit
            ...
            obiCheckerMP.NewStockDetected += async (sender, e) =>
            {
                ...
                var (Duration, DistanceKm) = await Drive.DisplayTravelTimeWithCacheAsync(e.Store.Address, e.Store.Name);
                var t = new BalloonNotifier("🚨 Midea disponible !", $"{obiCheckerMP.ProductName}\n{e.Store.Name}\n{Math.Floor(Duration.TotalHours)}h {Duration.Minutes}min pour {DistanceKm:F0} km \nStock: {e.NewQuantity}", url, e.Store.Name);
                t.Show();
            };
            ...
            obiCheckerMPC.NewStockDetected += async (sender, e) =>
            {
                ...
                var (Duration, DistanceKm) = await Drive.DisplayTravelTimeWithCacheAsync(e.Store.Address, e.Store.Name);
                var t = new BalloonNotifier("🚨 Midea disponible !", $"{obiCheckerMPC.ProductName}\n{e.Store.Name}\n{Math.Floor(Duration.TotalHours)}h {Duration.Minutes}min pour {DistanceKm:F0} km \nStock: {e.NewQuantity}", url, e.Store.Name);
                t.Show();
            };
